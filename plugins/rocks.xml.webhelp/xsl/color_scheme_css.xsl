...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="header-bg-color-value" select="if(normalize-space($header-bg-color)) then($header-bg-color) else('#0c213a')"/>
    <xsl:variable name="accent-bg-color-value" select="if(normalize-space($accent-bg-color)) then($accent-bg-color) else('#2c74eb')"/>
    <xsl:variable name="accent-color-value" select="if(normalize-space($accent-color)) then($accent-color) else('#2c74eb')"/>
